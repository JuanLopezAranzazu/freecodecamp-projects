...
function calculateChange(change) {
  ...
  for (let i = cid.length - 1; i >= 0; i--) {
    ...
    if (amountToReturn > 0) {
      result.push([currencyName, amountToReturn / 100]);
    }
  }
...
purchaseButton.addEventListener("click", () => {
  ...
  const totalDrawer = Math.round(getTotalDrawer() * 100) / 100;

  if (totalDrawer === change) {
    const result = cid.filter((currency) => currency[1] > 0).reverse();
  ...
  const { result, remaining } = calculateChange(change);
